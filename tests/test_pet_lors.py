...
def test_sinogram_axial_compression_operator_convention_drift_guard(
    ...
    """Cover the ``RuntimeError`` guard in ``_build_index_maps``.

    The guard catches a hypothetical divergence between the operator's
    internally computed plane multiplicity and the auto-built span-N
    descriptor's ``plane_multiplicity``.  Both currently use the same
    grouping logic, so this code path is unreachable in normal operation
    — but it's a safety net against silent convention drift if either side
    is ever modified independently.

    We force the guard to trip by monkey-patching
    :meth:`RegularPolygonPETLORDescriptor._setup_spanned_plane_indices` so
    that the companion span-3 descriptor reports inflated multiplicities,
    then assert that the operator's ``__init__`` raises.
    """
    ...
    def corrupted_setup(self):
        orig_setup(self)
        # Bump every multiplicity by 1 so np.array_equal fails inside the
        # operator's cross-check.
        self._plane_multiplicity = self._plane_multiplicity + 1
    ...
    with patch.object(
        ...
        with pytest.raises(RuntimeError, match="convention drift"):
            ppl.SinogramAxialCompressionOperator(lor_s1, target_span=3)
