"""
Transmission reconstruction: MLTR vs. SPS (separable paraboloidal surrogates)
=============================================================================
...
scatter_fraction = 0.1  # scatter relative to mean unscattered transmission
...
# -------
#
# Both algorithms converge to the same maximum-likelihood solution.  MLTR
# is typically the faster of the two; SPS additionally *guarantees* a
# monotone increase of :math:`L` (its optimal curvature is never smaller
# than the Newton curvature, hence the slightly more conservative steps).

fig, ax = plt.subplots(1, 2, figsize=(11, 4.5), tight_layout=True)
for name in algorithms:
    ax[0].semilogy(cost[name] - c_min + 1, label=name)
